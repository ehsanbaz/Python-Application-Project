...
def predict_linear_regression(stock_data, test_size):
    """
    Performs linear regression on stock data for predicting future stock prices.

    Args:
        stock_data (pandas.DataFrame): A DataFrame containing stock data with features like Open, High, Low, Volume, and Close.
        test_size (float): The proportion of the dataset to be used for testing (between 0 and 1).

    Returns:
        None. The function prints evaluation metrics, plots charts, and displays a forecast chart.
    """
    df = stock_data.copy()
    ...
    df_res = pd.DataFrame({"y_test": y_test, 'predicted': pred})

    def evaluation_metrics_print():
        st.markdown("# Evaluation Metrics")
        st.write(f'Mean Absolute Error (MAE): {mean_absolute_error(df_res["y_test"], df_res["predicted"])}')
        st.write(f'Mean Squared Error (MSE) : {mean_squared_error(df_res["y_test"], df_res["predicted"])}')
        st.write(f'Root Mean Squared Error (RMSE): {np.sqrt(mean_squared_error(df_res["y_test"], df_res["predicted"]))}')
    ...
    def charts_print():
        ...
        fig_preds = go.Figure()
        fig_preds.add_trace(go.Scatter(x=stock_data.index[train_size:], y=df_res['y_test'], mode='lines', name='Actual', line=dict(color='green')))
        fig_preds.add_trace(go.Scatter(x=stock_data.index[train_size:], y=df_res['predicted'], mode='lines', name='Predicted', line=dict(color='red')))
        ...
        df_res['residual'] = df_res['y_test'] - df_res['predicted']
        fig = px.scatter(df_res, x='predicted', y='residual', marginal_y='violin', trendline='ols', color='residual', color_continuous_scale=px.colors.diverging.Portland, labels={'x': 'Predicted Values', 'y': 'Residual'})
        ...
        df_res['residual'] = df_res['y_test'] - df_res['predicted']
        fig = px.scatter(df_res, x='y_test', y='predicted', marginal_x='histogram', marginal_y='histogram', trendline='ols', height=650, color='residual', color_discrete_sequence=px.colors.diverging.Portland, labels={'x': 'Actual Values', 'y': 'Predicted Values'})
        fig.update_traces(histnorm='probability', selector={'type': 'histogram'})
        fig.add_shape(type="line", line=dict(dash='dash', color='gray'), x0=df_res['y_test'].min(), y0=df_res['y_test'].min(), x1=df_res['y_test'].max(), y1=df_res['y_test'].max())
...
def predict_decision_tree_regression(stock_data, test_size, max_depth=None, min_samples_split=2, min_samples_leaf=1):
    """
    Performs decision tree regression on stock data for predicting future stock prices.

    Args:
        stock_data (pandas.DataFrame): A DataFrame containing stock data with features like Open, High, Low, Volume, and Close.
        test_size (float): The proportion of the dataset to be used for testing (between 0 and 1).
        max_depth (int, optional): The maximum depth of the decision tree. If None, then nodes are expanded until
    ...
    df_res = pd.DataFrame({"y_test": y_test_all, 'predicted': y_pred_all})
...
def predict_random_forest_regression(stock_data, test_size):
    """
    Performs random forest regression on stock data for predicting future stock prices.
